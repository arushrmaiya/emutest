...
int disassemble(unsigned char* buffer, int pc){
    ...
    switch(*code){
        ...
        default: printf("Invalid INSTR: %x\n", *code); break;
    }
    return opbytes;
}

int emulate(struct State8080_t* state){
    ...
    switch(*code){
        ...
        case 0x01: {
            ...
            state->regfile.b = code[2];
            state->regfile.c = code[1];
            state->pc += 2;
            break;
        }
        case 0x02: {
            printf("STAX B\n"); 
            uint16_t BC = state->regfile.b<<8 | state->regfile.c;
            state->memory[BC] = state->regfile.a;
            break;}
        case 0x03: {
            printf("INX B\n"); 
            uint16_t BC = (uint16_t)state->regfile.b<<8 | (uint16_t)state->regfile.c & 0x00ff;
            BC +=1;
            state->regfile.b = (uint8_t)BC >> 4;
            state->regfile.c = (uint8_t)BC & 0xff;
            break;}
        case 0x04: {
            printf("INR B\n"); 
            uint16_t result = (uint16_t)state->regfile.b + 1;
            state->regfile.b = result & 0xff; 
        ...
        case 0x06: {
            printf("MVI B, #%02x\n", code[1]);
            state->regfile.b = code[1];
            state->pc+=1;
            break;}
        case 0x07: {
            printf("RLC\n");
            uint8_t bit = (state->regfile.a >> 7) & 0x01;
            state->regfile.a = (state->regfile.a << 1) & 0xfe + bit;
            state->cc.cy = bit;
            break;}
        case 0x09: {
            printf("DAD B\n");
            uint16_t BC = state->regfile.b << 8 | state->regfile.c;
            uint16_t HL = state->regfile.h << 8 | state->regfile.l;
            uint32_t result = (uint32_t)BC + (uint32_t)HL;
            state->cc.cy = (result > 0xffff);
            state->regfile.h = (result >> 8) & 0xff;
            state->regfile.l = result & 0xff;
            break;}
        case 0x0a: {
            printf("LDAX B\n");
            uint16_t BC = state->regfile.b << 8 | state->regfile.c;
            state->regfile.a = state->memory[BC];
            break;}
        case 0x0b: {
            printf("DCX B\n");
            uint16_t BC = state->regfile.b << 8 | state->regfile.c;
            BC = BC - 1;
            state->regfile.b = (BC >> 8) & 0xff;
            state->regfile.c = BC & 0xff; 
            break;}
        case 0x0c: {
            printf("INR C\n"); 
            uint16_t result = (uint16_t)state->regfile.c + 1;
            state->regfile.c = result & 0xff;
            state->cc.z = ((result & 0xff) == 0);
            state->cc.s = ((result & 0x80) != 0);
            /*TODO: Implement Parity*/
            break;}
        ...
        case 0x40: printf("MOV B, B\n"); break;
        case 0x41: printf("MOV B, C\n");state->regfile.b = state->regfile.c; break;
        case 0x42: printf("MOV B, D\n");state->regfile.b = state->regfile.d; break;
        case 0x43: printf("MOV B, E\n");state->regfile.b = state->regfile.e; break;
        case 0x44: printf("MOV B, H\n");state->regfile.b = state->regfile.h; break;
        case 0x45: printf("MOV B, L\n");state->regfile.b = state->regfile.l; break;
        case 0x46: printf("MOV B, M\n"); break;
        case 0x47: printf("MOV B, A\n"); state->regfile.b = state->regfile.a; break;
        case 0x48: printf("MOV C, B\n"); state->regfile.c = state->regfile.b; break;
        case 0x49: printf("MOV C, C\n"); break;
        case 0x4a: printf("MOV C, D\n"); break;
        case 0x4b: printf("MOV C, E\n"); break;
        case 0x4c: printf("MOV C, H\n"); break;
